...
int readelf(const void *binary, size_t size) {
	...
	// Get the address of the section header table, the number of section headers and the size of a
	// section header.
	const void *sh_table;
	Elf32_Half sh_entry_count;
	Elf32_Half sh_entry_size;
	/* Exercise 1.1: Your code here. (1/2) */
	sh_table = (const void *)(binary + ehdr->e_shoff);
	...
	for (int i = 0; i < sh_entry_count; i++) {
		...
		addr = (shdr + i)->sh_addr; // (Elf32_Shdr *)(sh_table + i * sh_entry_size)
